...
export class YourFulfillmentAdapter implements IFulfillmentAdapter {
    ...
    try {
      ...
      if (!response.success) {
        throw new AdapterError(
          'Failed to connect to YourFulfillment',
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      throw new AdapterError(
        `Disconnect failed: ${getErrorMessage(error)}`,
  ...
  async captureOrder(params: OrderRequest): Promise<OrderResult> {
    try {
      ...
      throw new AdapterError(
        `Order capture failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async updateOrder(orderId: string, updates: OrderUpdates): Promise<UpdateResult> {
    try {
      ...
      throw new AdapterError(
        `Order update failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async returnOrder(orderId: string, items: ReturnItem[]): Promise<ReturnResult> {
    try {
      ...
      const response = await this.client.post(`/orders/${orderId}/returns`, {
      ...
      return {
        success: true,
        returnId: (response.data as any)?.return_id,
        rmaNumber: (response.data as any)?.rma_number,
        status: 'pending',
        refundAmount: (response.data as any)?.refund_amount,
        returnLabel: (response.data as any)?.return_label_url,
  ...
  async exchangeOrder(params: ExchangeParams): Promise<ExchangeResult> {
    try {
      ...
      const response = await this.client.post(`/orders/${params.orderId}/exchanges`, {
        return_items: params.returnItems,
      ...
      return {
        success: true,
        exchangeId: (response.data as any)?.exchange_id,
        originalOrderId: params.orderId,
        newOrderId: (response.data as any)?.new_order_id,
        priceDifference: (response.data as any)?.price_difference,
        rmaNumber: (response.data as any)?.rma_number,
        message: `Exchange created for order ${params.orderId}`
      };
    } catch (error: unknown) {
      if (error instanceof AdapterError) {
        throw error;
      ...
      throw new AdapterError(
        `Exchange creation failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async shipOrder(orderId: string, shipping: ShippingInfo): Promise<ShipmentResult> {
    try {
      ...
      throw new AdapterError(
        `Shipment creation failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async holdOrder(orderId: string, holdParams: HoldParams): Promise<HoldResult> {
    try {
      ...
      throw new AdapterError(
        `Hold order failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async splitOrder(orderId: string, splits: SplitParams[]): Promise<SplitResult> {
    try {
      ...
      throw new AdapterError(
        `Split order failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async reserveInventory(reservation: ReservationRequest): Promise<ReservationResult> {
    try {
      const response = await this.client.post('/inventory/reserve', {
        ...
        duration_minutes: reservation.expiresInMinutes || 15,
        created_at: new Date().toISOString()
      });
      
      if (!response.success) {
        // Check for insufficient inventory
        if (response.error?.code === 'INSUFFICIENT_INVENTORY') {
          const details = response.error.details;
          if (details && typeof details === 'object' && 'sku' in details && 'requested' in details && 'available' in details) {
            throw new InsufficientInventoryError(
              String(details.sku),
              Number(details.requested),
              Number(details.available)
            );
          }
        }
        throw new AdapterError(
      ...
      return {
        success: true,
        reservationId: (response.data as any)?.reservation_id,
        items: ((response.data as any)?.items || []).map((item: any) => ({
          sku: item.sku,
          reserved: item.reserved,
          available: item.available
        })),
        expiresAt: (response.data as any)?.expires_at,
      ...
      throw new AdapterError(
        `Inventory reservation failed: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async getOrder(identifier: OrderIdentifier): Promise<Order> {
    try {
      ...
      throw new AdapterError(
        `Failed to get order: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async getInventory(sku: string, locationId?: string): Promise<Inventory> {
    try {
      ...
      throw new AdapterError(
        `Failed to get inventory: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async getProduct(identifier: ProductIdentifier): Promise<Product> {
    try {
      ...
      throw new AdapterError(
        `Failed to get product: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async getShipment(identifier: ShipmentIdentifier): Promise<Shipment> {
    try {
      ...
      if (!response.success) {
        throw new AdapterError(
          `Shipment not found: ${JSON.stringify(identifier)}`,
          ErrorCode.API_ERROR,
          identifier
        );
      }
      ...
      throw new AdapterError(
        `Failed to get shipment: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  async getBuyer(buyerId: string): Promise<Buyer> {
    try {
      ...
      throw new AdapterError(
        `Failed to get buyer: ${getErrorMessage(error)}`,
        ErrorCode.UNKNOWN_ERROR,
        error
      );
    }
  ...
  private transformToShipment(fulfillmentShipment: YourFulfillmentShipment): Shipment {
    return {
      ...
      shippingAddress: fulfillmentShipment.to_address ? { // Required field
