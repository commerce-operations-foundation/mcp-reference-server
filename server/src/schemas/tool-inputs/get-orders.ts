...
export const GetOrdersInputSchema = z
  .object({
    orderIds: z.array(z.string()).describe('Internal order ID, could be a comma separated list'),
    extOrderIds: z.array(z.string()).describe('External order ID from source system, could be a comma separated list'),
    statuses: z.array(z.string()).describe('Order status'),
    orderNames: z.array(z.string()).describe('Friendly Order identifier'),
