...
import { vi, describe, it, expect, beforeEach, afterEach, beforeAll, afterAll } from 'vitest';import { ConfigLoader } from '../../src/config/config-loader';
...
describe('ConfigManager', () => {
  ...
  beforeEach(() => {
    ...
    (ConfigLoader as vi.MockedClass<typeof ConfigLoader>).mockImplementation(() => mockLoader);
    
    // Mock ConfigValidator
    mockValidator = {
      validate: vi.fn()
    } as any;
    (ConfigValidator as vi.MockedClass<typeof ConfigValidator>).mockImplementation(() => mockValidator);
    
    // Mock EnvironmentConfig
    (EnvironmentConfig.load as vi.Mock).mockReturnValue({});
  ...
  describe('reload', () => {
    it('should reload configuration', () => {
      const newConfig = { ...mockConfig, server: { ...mockConfig.server, port: 5000 } };
    ...
    it('should detect critical changes', async () => {
      const newConfig = { ...mockConfig, adapter: { type: 'npm' as const, name: 'different', config: {} } };
      mockLoader.loadDefaults.mockReturnValue(newConfig);
