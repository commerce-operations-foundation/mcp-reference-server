import { serverConfigSchema, ServerConfig } from '../../src/types/config';
  ...
  describe('validate', () => {
    ...
    describe('server validation', () => {
      it('should require server configuration', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow();
      });

      it('should require server name', () => {
        (validConfig.server as any).name = undefined;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Server name is required');
      });

      it('should require valid environment', () => {
        (validConfig.server as any).environment = 'invalid';
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Server environment must be one of');
      });

      it('should validate port range', () => {
        validConfig.server.port = 70000;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Server port must be a number between 1 and 65535');
      });
    });
    ...
    describe('adapter validation', () => {
      ...
      it('should require adapter name for built-in type', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Built-in adapter requires a name');
      });
    });
    ...
    describe('logging validation', () => {
      ...
      it('should require valid log level', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Logging level must be one of');
      });

      it('should require logging directory', () => {
        (validConfig.logging as any).dir = undefined;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Logging directory is required');
      });
      ...
      it('should validate external logging when enabled', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('External logging service is required');
      });
    });
    ...
    describe('security validation', () => {
      ...
      it('should validate sanitization max request size', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Max request size must be a positive number');
      });
    });
    ...
    describe('performance validation', () => {
      ...
      it('should validate monitoring interval', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Monitoring interval must be at least 1000ms');
      });

      it('should validate request timeout', () => {
        validConfig.performance.timeout.request = 0;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Request timeout must be a positive number');
      });
      ...
      it('should ensure adapter timeout is less than request timeout', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Adapter timeout must be less than request timeout');
      });
    });
    ...
    describe('resilience validation', () => {
      ...
      it('should validate circuit breaker failure threshold', () => {
        ...
        expect(() => serverConfigSchema.parse(validConfig)).toThrow(
          'Circuit breaker failure threshold must be a positive number'
        );
      });

      it('should validate circuit breaker reset timeout', () => {
        validConfig.resilience.circuitBreaker.resetTimeout = 500;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Circuit breaker reset timeout must be at least 1000ms');
      });

      it('should validate retry max attempts', () => {
        validConfig.resilience.retry.maxAttempts = 0;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Retry max attempts must be a positive number');
      });

      it('should validate retry initial delay', () => {
        validConfig.resilience.retry.initialDelay = -1;
        expect(() => serverConfigSchema.parse(validConfig)).toThrow('Retry initial delay must be non-negative');
      });
    });
  });
});
